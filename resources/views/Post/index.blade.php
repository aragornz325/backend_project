        ...
        <tbody>
            @if (count($posts) >= 1 )
                ...
                <tr>
                    ...
                    <td>{{ $post->author }}</td>
                    <td>
                        <a href="{{ route('post.show', $post->id)}}" class="btn btn-sm btn-outline-primary">Leer</a><br>
                        <a href="{{ route("post.edit",$post->id ) }}" class="btn btm-sm btn-outline-info">Actualizar </a>
                        <form action="{{ route("post.destroy", $post->id) }}" method="post">
                            {{ csrf_field() }}
                            {{ method_field("DELETE") }}
                            <button type="submit" class="btn btn-sm btn-outline-danger" onclick="return confirm('estas seguro?, si clickeas ACEPTAR lo hago mierda!!!')">Hacer mierda</button>
